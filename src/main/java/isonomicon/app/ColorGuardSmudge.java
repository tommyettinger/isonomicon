...
public class ColorGuardSmudge extends ApplicationAdapter {
...
//    public static final String outDir = "out/cg_smudge";
//    public static final String outDir = "out/cg_July_23_2024";
//    public static final String outDir = "out/cg_smudge_Gourd_0_3";
//    public static final String outDir = "out/cg_smudge_Banter_0_7";
//    public static final String outDir = "out/cg_smudge_Banter_0_4";
    public static final String outDir = "out/cg_smudge_Banter_0_3";
...
//        gif = new LoafGif();
            ...
            gif.palette = new QualityPalette(); // Uses Snuggly255
...
//                    renderer.shadows = true;
                ...
                Texture t = new Texture(renderer.pixmap.getWidth(), renderer.pixmap.getHeight(), Pixmap.Format.RGBA8888);
                Pixmap pixmap;
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    for (int f = 0; f < 4; f++) {
                        ...
                        renderer.drawSplats(VoxIOExtended.mergeModel(voxels), i * 0.25f, 0f, 0f, f, 0.00f, 0.00f, 0.00f, Stuff.MATERIALS_B);
                        t.draw(renderer.pixmap, 0, 0);
...
//                                continue;
                            for (int j = 0; j < armies.length; j++) {
                                ...
                                if(look + j == 0)
                                    if(png != null) png.write(Gdx.files.local(outDir + "/lab/" + name + '/' + name + "_angle" + i + "_" + f + ".png"), renderer.pixmap);
                ...
                if (ATTACKS) {
                    ...
                    for (int which = 0; which < 2; which++) {
                        ...
                        for (int i = 0; i < 4; i++) {
                            ...
                            for (int f = 0; f < frames.length; f++) {
                                renderer.drawSplats(VoxIOExtended.mergeModel(frames[f]), i * 0.25f, 0f, 0f, f, 0.00f, 0.00f, 0.00f, Stuff.MATERIALS_B);
                                t.draw(renderer.pixmap, 0, 0);
                                for (int look = 0, lk = 0; look < 153; look+=8, lk++) {
                                    for (int j = 0; j < armies.length; j++) {
                                        fb.begin();
                                        ...
                                        if (look + j == 0)
                                            if(png != null) png.write(Gdx.files.local(outDir + "/lab/" + name + '/' + name + ps + "_angle" + i + "_" + f + ".png"), renderer.pixmap);
                                        if(lk == canonicalLooks[j]) {
                                            pm.set(j * 32 + i * 8 + f, pixmap);
                                        } else {
                                            pixmap.dispose();
                                    ...
                                    for (int i = 0; i < 4; i++) {
                                        ...
                                        for (int f = 0; f < frames.length; f++) {
                                            ...
                                            for (int j = 0; j < armies.length; j++) {
                                                ...
                                                if (look + j == 0)
                                                    if(png != null) png.write(Gdx.files.local(outDir + "/lab/" + attack + "_Receive/" + attack + "_Receive_" + strength + "_angle" + i + "_" + f + ".png"), renderer.pixmap);
                                            }
                                        }
                                    }
...
//            SpecialRenderer.shrink = 1;
            renderer = new SmudgeRenderer(120);
...
//            renderer.shadows = false;
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    for (int f = 0; f < 1; f++) {
                        ...
                        try {
                            ...
                            if (n == 0)
                                if(png != null) png.write(Gdx.files.local(outDir + "/lab/Landscape/" + name + "_angle" + i + "_" + f + ".png"), renderer.pixmap);
                        } finally {
                            pixmap.dispose();
                        }
                    }
    ...
    public static void main(String[] arg) {
        ...
        final ColorGuardSmudge app = new ColorGuardSmudge();
