 ...
 */
public class SpecialRenderer2025 {
...
//    public static float distortHXY = 2, distortVXY = 0.5f, distortVZ = 3; // partially elevated side view ("shallow")
    ...
    protected SpecialRenderer2025() {
        this(64);
    }

    public SpecialRenderer2025(final int size) {
        this(size, Stuff.STUFFS);
    }

    public SpecialRenderer2025(final int size, Stuff[] stuffs) {
        this.size = size;
     ...
     */
    public SpecialRenderer2025 saturation(float saturationModifier) {
    ...
    public SpecialRenderer2025 palette(PaletteReducer color) {
        return palette(color.paletteArray, color.colorCount);
    }

    public SpecialRenderer2025 palette(int[] color) {
        return palette(color, 256);
    }
    public SpecialRenderer2025 palette(int[] color, int count) {
        this.palette = color;
...
//                    }
    ...
    public SpecialRenderer2025 clear() {
...
//                    int shade = (int) (logisticky((shading[x][y] + midShading[x][y])) * 255.999f);
                    int shade = (int) (Math.min(Math.max((shading[x][y] + logisticky(midShading[x][y])) * 0.625f + 0.5f, 0f), 1f) * 255.999f);
...
//                    int shade = (int) (logisticky((shading[x][y] + midShading[x][y])) * 255.999f);
                    int shade = (int) (1f - Math.min(Math.max((shading[x][y] + logisticky(midShading[x][y])) * 0.625f + 0.5f, 0f), 1f) * 255.999f);
//                    int shade = (int) ((1f - Math.min(Math.max((shading[x][y] + midShading[x][y]) * 0.625f + 0.1328125f, 0f), 1f)) * 255.999f);
                    int idx = (y >>> shrink) * palettePixmap.getWidth() + (x >>> shrink) << 2;
                    if ((buffer.get(idx+3) & 255) < shade) {
